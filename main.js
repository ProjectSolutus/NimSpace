...
function autoStackCollapseOnPeers() {
  ...
  // If on landing page (startBtn visible), always expand
...
function broadcastData(msg, exceptPeerId = null) {
  ...
  if (msg && msg.type === 'file') {
    if (!chatHistory.find(m => m.type === 'file' && m.fileMsgId === msg.fileMsgId)) {
...
if (joinPeerId) { hide(startBtn); joinMesh(); }
...
function renderBingeSourceForm() {
  ...
  form.addEventListener('submit', e => {
    ...
    if (type === 'url') {
      ...
      startBingeSession({ type: embedInfo.type, value: embedInfo.embedUrl, originalUrl: url });
    } else if (type === 'file') {
      ...
      startBingeSession({ type: 'file', value: file });
      ...
      if (!window.WebTorrent) {
        ...
        script.onload = () => handleMagnetBinge(magnet);
        document.body.appendChild(script);
      } else {
        handleMagnetBinge(magnet);
      }
    }
  });
}
...
function handleMagnetBinge(magnet) {
  const area = document.getElementById('bingeSourceFormArea');
  if (!window.WebTorrent) {
  ...
  try {
    ...
    if (window._bingeWebTorrent) {
      window._bingeWebTorrent.destroy();
    }
    window._bingeWebTorrent = new WebTorrent(getWebTorrentConfig(true));
    const client = window._bingeWebTorrent;
    ...
    client.add(magnet, getWebTorrentConfig(), torrent => {
      ...
      file.getBlobURL((err, url) => {
        ...
        if (hostVideoElem) {
          ...
          window._bingeCurrentTorrent = torrent;
          window._bingeCurrentFile = file;
          window._bingeHostBlobUrl = url;
        }
      });
    });
  } catch (err) {
...
function updateHostTorrentSeeding(paused) {
  if (window._bingeWebTorrent && window._bingeWebTorrent.torrents && window._bingeWebTorrent.torrents[0]) {
    const t = window._bingeWebTorrent.torrents[0];
    if (paused) {
      t.pause();
...
function updatePeerTorrentDownloading(paused) {
  if (window.bingeTorrent && window.bingeTorrent.torrents && window.bingeTorrent.torrents[0]) {
    const t = window.bingeTorrent.torrents[0];
    if (paused) {
      t.pause();
...
function renderBingePlayer() {
  ...
  if (type === 'url' || type === 'video') { // Direct video/audio URL
    ...
    if (isHost && window.bingeHostBlobUrl) {
      videoSrc = window.bingeHostBlobUrl;
  ...
  if (type === 'youtube') {
    ...
    if (isPeer && (type === 'file' || type === 'torrent')) {
      ...
      if (!window.WebTorrent) {
        const script = document.createElement('script');
        script.src = 'https://cdn.jsdelivr.net/npm/webtorrent@latest/webtorrent.min.js';
        script.onload = () => setupPeerWebTorrentVideo(video, magnet, debugPrefix);
        document.body.appendChild(script);
        console.log(`${debugPrefix} Loading WebTorrent library...`);
      } else {
...
function setupPeerWebTorrentVideo(video, magnet, debugPrefix) {
  try {
    debugMagnetStream('peer', 'Setting up WebTorrent video player', { magnet });
    
    if (!window.WebTorrent) {
      debugMagnetStream('peer', 'ERROR: WebTorrent not loaded');
      if (video && video.parentElement) {
        video.parentElement.innerHTML += '<div style="color:#b14a4a;padding:1em;">WebTorrent failed to load. Please reload the page.</div>';
      }
      return;
    }

    // Clear any existing client to prevent memory leaks
    if (window.bingeTorrent && !window.bingeTorrent.destroyed) {
      debugMagnetStream('peer', 'Destroying existing WebTorrent client');
      window.bingeTorrent.destroy();
    }

    window.bingeTorrent = new WebTorrent(getWebTorrentConfig(true));
    debugMagnetStream('peer', 'Created new WebTorrent client');

    // Add connection error handling
    window.bingeTorrent.on('error', function(err) {
      console.error('[MAGNET][PEER] Client error:', err);
      // Try to recover from error
      setTimeout(() => {
        if (!window.bingeTorrent.destroyed) {
          debugMagnetStream('peer', 'Attempting to recover from error');
          window.bingeTorrent.destroy();
          window.bingeTorrent = new WebTorrent(getWebTorrentConfig(true));
          window.bingeTorrent.add(magnet, getWebTorrentConfig(), onTorrent);
        }
      }, 1000);
    });
    ...
    function onTorrent(torrent) {
      ...
      torrent.on('download', function(bytes) {
        ...
        if (currentProgress - lastProgress >= 1 || currentProgress === 100) {
          ...
          if (document.getElementById('video-loading-indicator')) {
            document.getElementById('video-loading-indicator').textContent = 
      ...
      torrent.on('done', () => {
        ...
        if (document.getElementById('video-loading-indicator')) {
          document.getElementById('video-loading-indicator').remove();
        }
      });
    }
  ...
  peer.on('call', async (mediaCall) => {
    ...
    try {
      ...
      mediaCall.on('stream', (remoteStream) => {
        ...
        if (window.bingeTorrent && typeof window.bingeTorrent.destroy === 'function') {
          try {
            console.log('[BINGE][PEER] Received live broadcast – shutting down WebTorrent');
            window.bingeTorrent.destroy(() => { console.log('[BINGE][PEER] WebTorrent client destroyed'); });
            window.bingeTorrent = null;
          } catch (e) {
...
function startBingeSession(source) {
  ...
  if (source.type === 'file') {
    ...
    if (!window.WebTorrent) {
      const script = document.createElement('script');
      script.src = 'https://cdn.jsdelivr.net/npm/webtorrent@latest/webtorrent.min.js';
      script.onload = () => seedBingeFileWithWebTorrent(bingeFileObj);
    } else {
...
function seedBingeFileWithWebTorrent(file) {
  ...
  if (window._bingeWebTorrent) { window._bingeWebTorrent.destroy(); window._bingeWebTorrent = null; }
  console.log('[BINGE] Seeding file:', file);
  const client = new WebTorrent({ announce: WEBTORRENT_TRACKERS });
  window._bingeWebTorrent = client;
...
function handleBingeData(data, fromPid) {
  ...
  if (data.type === 'binge-start') {
    ...
    if (bingeHostId === myPeerId) {
      const video = document.getElementById('bingeVideo');
      if (video) sendBingeSyncToPeer(fromPid, video.currentTime, video.paused);
...
function setupTorrentPlayer() {
  ...
  if (window._bingeTorrentPlayerInit) {
  ...
  window._bingeTorrentPlayerInit = true;
  ...
  if (!window.WebTorrent) {
    if (!window._webtorrentLoading) {
      window._webtorrentLoading = true;
      console.warn('[BINGE][DEBUG] WebTorrent is not loaded! Loading script...');
      const script = document.createElement('script');
      script.src = 'https://cdn.jsdelivr.net/npm/webtorrent@latest/webtorrent.min.js';
      script.onload = () => {
        console.log('[BINGE][DEBUG] WebTorrent script loaded! Retrying setupTorrentPlayer...');
        setTimeout(() => { setupTorrentPlayer(); }, 0);
      };
      document.body.appendChild(script);
    } else {
      console.warn('[BINGE][DEBUG] WebTorrent script is loading, will retry shortly...');
      setTimeout(() => { setupTorrentPlayer(); }, 300);
    }
    return;
  }
...
window.showBingeView = showBingeView;
...
function cleanupAllMediaResources() {
  ...
  if (window._bingeWebTorrent && typeof window._bingeWebTorrent.destroy === 'function') {
    console.log('[BINGE] Cleaning up WebTorrent client (host)');
    try {
      window._bingeWebTorrent.destroy(() => {
        console.log('[BINGE] WebTorrent client (host) destroyed');
        window._bingeWebTorrent = null;
      });
    } catch (e) {
...
function startHostPeerDebug() {
  if (window._bingeWebTorrent && window._bingeWebTorrent.torrents && window._bingeWebTorrent.torrents[0]) {
    setInterval(() => {
      const n = window._bingeWebTorrent.torrents[0]?.numPeers;
...
function startPeerPeerDebug() {
  if (window.bingeTorrent && bingeTorrent.torrents && bingeTorrent.torrents[0]) {
    setInterval(() => {
...
function applyBingeSyncToPeer(sync) {
  ...
  if (video.readyState < 2) { // HAVE_CURRENT_DATA
    ...
    if (!loadingMsg && video.parentElement) {
      ...
      loadingMsg.style.cssText = 'color:#888;padding:1em;text-align:center;';
      loadingMsg.textContent = 'Loading video...';
      video.parentElement.appendChild(loadingMsg);
    }
    console.log('[BINGE][DEBUG][PEER] Video not ready, queueing sync');
    ...
    if (!video._syncPollInterval) {
      video._syncPollInterval = setInterval(() => {
        if (video.readyState >= 2 && pendingBingeSync) {
...
function actuallyApplyBingeSyncToPeer(sync) {
  const video = document.getElementById('bingeVideo');
  if (!video) return;
  // Always seek to host's current time if difference > 0.2s
  if (Math.abs(video.currentTime - sync.currentTime) > 0.2) {
  ...
  if (sync.paused && !video.paused) {
    ...
    // Show loading if paused and not ready
    ...
    // Show loading while waiting for play
    let loadingMsg = document.getElementById('bingePeerLoadingMsg');
    if (!loadingMsg && video.parentElement) {
      loadingMsg = document.createElement('div');
      loadingMsg.id = 'bingePeerLoadingMsg';
    ...
    const tryPlay = () => {
      if (video.readyState >= 2) {
        video.play().then(() => {
          // Remove loading message when playback starts
...
// Patch handleBingeData for peer sync
const origHandleBingeData = handleBingeData;
handleBingeData = function(data, fromPid) {
  if (data.type === 'binge-sync') {
    if (bingeHostId !== data.hostId) return;
    if (bingeHostId !== myPeerId) {
      applyBingeSyncToPeer(data);
  ...
  if (data.type === 'binge-start') {
    ...
    // Only re-render Binge UI if visible
    if (isBingeScreenVisible()) renderBingeUI();
    return;
  }
  if (data.type === 'binge-stop') {
    const area = document.getElementById('bingeBannerArea');
    if (area) {
      area.innerHTML = `<div class="binge-banner">Binge session ended by host.</div>`;
    }
  ...
  origHandleBingeData(data, fromPid);
};
...
const origRenderBingePlayer2 = renderBingePlayer;
renderBingePlayer = function() {
  origRenderBingePlayer2();
...
const origRenderBingePlayer3 = renderBingePlayer;
renderBingePlayer = function() {
  origRenderBingePlayer3();
...
function setupAmbientBackgroundEffect(element) {
  ...
  const videoContainer = element.closest('.binge-player');
  if (!videoContainer) {
    console.log('[AMBIENT] Could not find video container');
    return;
  }

  const ambientBg = createVideoAmbientBackground(videoContainer);
...
function createVideoAmbientBackground(videoContainer) {
  console.log('[AMBIENT] Creating ambient background div in container:', videoContainer);
  const ambientBg = document.createElement('div');
  ambientBg.className = 'binge-ambient-bg';
  videoContainer.insertBefore(ambientBg, videoContainer.firstChild);
  return ambientBg;
}
